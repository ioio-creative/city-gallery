...
const Map = props => {
  ...
  useEffect(() => {
    ...
    const mapAssets = data.hh.mapAssets;
    const options = {
      progress: 0,
      year: {
        y1900: 0,
    ...
    const preload = () => {
      ...
      loader.load((loader, resources) => {
        ...
        for (let s = 0, lth = data.hh.streets.length; s < lth; s++) {
          streets[s] = [];
          for (let i = 0; i < data.hh.streets[s].length; i++) {
            const street = data.hh.streets[s][i];
            streets[s][i] = new Marker(street);
            streets[s][i].create();
          }
        }

        for (let i = 0, lth = data.hh.coastlineParts.length; i < lth; i++) {
          const parts = data.hh.coastlineParts[i];
