...
// import diff from './images/kc_diffuse.svg'
// import start_diff from './images/kc_start_diffuse.svg'
// import old_diff from './images/kc_old_diffuse.svg'
// import mask from './images/kc_mask.svg'
// import coastline1900_mask from './images/kc_coastline1900_mask.svg'
// import coastline1945_mask from './images/kc_coastline1945_mask.svg'
// import coastline1985_mask from './images/kc_coastline1985_mask.svg'
// import coastline2019_mask from './images/kc_coastline2019_mask.svg'
...
const Map = props => {
  ...
  useEffect(() => {
    ...
    const mapAssets = data.kc.mapAssets;
    const options = {
      progress: 0,
      year: {
        y1900: 0,
      ...
      useNoiseTxt: function () {
        map['kc'].image.shader.uniforms.noiseTexture = new PIXI.Sprite.from('noise').texture.baseTexture;
      },
      useSandTxt: function () {
        map['kc'].image.shader.uniforms.noiseTexture = new PIXI.Sprite.from('sand').texture.baseTexture;
      }
    };
    ...
    class Map {
      ...
      create() {
        ...
        for (let i = 0; i < years.length; i++) {
          const year = years[i];
          this.highlight[i] = new PIXI.Sprite.from(`kc_coastline${year}_highlight`);
      ...
      createShader() {
        const shader = new PIXI.Shader.from(
          ...
          `,
          {
            ...
            startDiffuse: new PIXI.Sprite.from('kc_start_diffuse').texture.baseTexture,
            oldDiffuse: new PIXI.Sprite.from('kc_old_diffuse').texture.baseTexture,
            maskTexture: new PIXI.Sprite.from('kc_mask').texture.baseTexture,
            coastline1900Mask: new PIXI.Sprite.from('kc_coastline1900_mask').texture.baseTexture,
            coastline1945Mask: new PIXI.Sprite.from('kc_coastline1945_mask').texture.baseTexture,
            coastline1985Mask: new PIXI.Sprite.from('kc_coastline1985_mask').texture.baseTexture,
            coastline2019Mask: new PIXI.Sprite.from('kc_coastline2019_mask').texture.baseTexture,
    ...
    const preload = () => {
      ...
      loader.load((loader, resources) => {
        // create map
        map['kc'] = new Map(resources['kc_diffuse'].texture);
        map['kc'].create();

        for (let s = 0, lth = data.kc.streets.length; s < lth; s++) {
          streets[s] = [];
          for (let i = 0; i < data.kc.streets[s].length; i++) {
            const street = data.kc.streets[s][i];
            streets[s][i] = new Marker(street);
            streets[s][i].create();
          }
        }

        for (let i = 0, lth = data.kc.coastlineParts.length; i < lth; i++) {
          const parts = data.kc.coastlineParts[i];
    ...
    const startAnim = () => {
      app.ticker.add(delta => {
        if (map['kc']) {
          map['kc'].image.shader.uniforms.progress = options.progress;
          map['kc'].image.shader.uniforms.progress1900 = options.year.y1900;
          map['kc'].image.shader.uniforms.progress1945 = options.year.y1945;
          map['kc'].image.shader.uniforms.progress1985 = options.year.y1985;
          map['kc'].image.shader.uniforms.progress2019 = options.year.y2019;
          map['kc'].image.shader.uniforms.threshold = options.threshold;
        }
      });
    };
    ...
    const moveMap = (idx = 0) => {
      gsap.to([mapContainer, markerContainer, coastlinePartsContainer], 1, {
        x: zoomedPos.x - idx * (map['kc'].image.width / (4 - 1)),
        y: zoomedPos.y,
        ease: 'power3.inOut'
      });
      // gsap.to('#markerOuterWrap', 1, {x:zoomedPos.x - (idx * (map['kc'].image.width/(4-1))), y:zoomedPos.y, ease:'power3.inOut'});
    };
    ...
    const selectCoastline = idx => {
      const highlights = map['kc'].highlight;
      selectedHighlight = map['kc'].highlight[idx];
